...
$.ajax({
  ...
  data: {
    key: "RNJBZ-3GI64-ADTUQ-XAM3T-P67NF-SQFXP", // 这里要写你的KEY!!!
    output: "jsonp",
  },
  dataType: "jsonp",
...
function addAnchor() {
  ...
  txt = txt.replace(/<[^>]*>/g, "");
  $("#post-anchor-box").prepend(
    '<a class="anchor-item" href="javascript:void(0);" onclick="toAnchor(this)" data-pjax-state="">' +
      txt +
      "</a>"
  );
  debounce(function () {
    ...
    $.fn.nekoScroll = function (option) {
      ...
      $(window).scroll(function () {
        ...
        if (basicInfo.ScrollTop == basicInfo.S_V) {
          ...
          console.log(basicInfo.ScrollTop, basicInfo.S_V);
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by WEI %c 你正在访问WEIの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| WEI")
...
if (m == 6 && dd == 30) {
  //小猫咪生日
  if (sessionStorage.getItem("isPopupWindow") != "1") {
    Swal.fire("祝小猫咪" + (y - 2022).toString() + "岁生日快乐！🐱");
    sessionStorage.setItem("isPopupWindow", "1");
  }
}
...
class Cursor {
  ...
  create() {
    ...
    this.scr.innerHTML =
      `* {cursor: url("data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 8 8' width='10px' height='10px'><circle cx='4' cy='4' r='4' opacity='1.0' fill='` +
      colorVal +
      `'/></svg>") 4 4, auto}`;
  }
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://s1.imagehub.cc/images/2025/03/12/4e9fcd6059afdba2dfcb8cfbbaf0cfea.webp);
    --darkmode-bg:url(https://s1.imagehub.cc/images/2025/03/12/18cfa4c3e32b11131ec22fbf61b33be0.webp);
    --mobileday-bg: url(https://s1.imagehub.cc/images/2025/03/12/4628b02354a5a1b8c08e1216ac610d15.webp);
    --mobilenight-bg: url(https://s1.imagehub.cc/images/2025/03/12/572f5706315acb560b1041a48e95244d.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
function getPicture_(url) {
  checkImgExists(url ? url : document.getElementById("pic-link").value)
    .then(() => {
      // 有效的图片链接
      var link = `url(${
        url ? url : document.getElementById("pic-link").value
      })`;
      changeBg(link);
      // 提示切换成功
      new Vue({
...
<p id="swfs">
<a class="swf" id="swf_ZhuZiAWan" href="javascript:;" rel="noopener external nofollow" style="font-family:'ZhuZiAWan'!important;color:black" onclick="setFont('ZhuZiAWan')">筑紫丸标准体</a>
...
<details class="folding-tag" cyan><summary> 查看适配手机的背景 </summary>
              <div class='content'>
              <div class="bgbox"><a href="javascript:;" rel="noopener external nofollow" style="background-image:url(https://t.lizi.moe/mp)" class="pimgbox" onclick="changeBg('url(https://t.lizi.moe/mp)')"></a></div>
              </div>
            </details>
...
function bindVideoControls() {
  const e = document.getElementById("card_girl_video");
  if (!e) return;
  const t = `http://api.mmp.cc/api/ksvideo?type=mp4&id=GaoZhiLiangXiaoJieJie`,
    o = $(".girl_btn_fuck"),
    i = $(".girl_btn_next"),
    n = () => {
      (e.src = t),
        e.play().catch((e) => console.error("Video play failed:", e)),
        o.text("暂停");
    };
  o.off("click").on("click", () => {
    e.paused ? (e.play(), o.text("暂停")) : (e.pause(), o.text("播放"));
  }),
    i.off("click").on("click", n),
...
function lazyLoadVideo() {
  const e = document.getElementById("card_girl_video");
  if (!e) return; // 如果元素不存在，直接退出
  // 定义视频资源的 URL（这里是一个固定的接口地址）
  const t = `http://api.mmp.cc/api/ksvideo?type=mp4&id=GaoZhiLiangXiaoJieJie`;
  // 将视频 URL 存储到自定义属性 data-src 中（暂存，不立即加载）
  e.setAttribute("data-src", t);
  const o = new IntersectionObserver(
    (entries) => {
      // 遍历所有被观察的元素（这里只观察了视频元素 e）
      entries.forEach((entry) => {
        // 当元素进入视口（isIntersecting 为 true）时
        if (entry.isIntersecting) {
          // 将 data-src 中暂存的 URL 赋值给 video 的 src 属性（触发加载）
          e.src = e.getAttribute("data-src");
          // 手动触发视频加载（部分浏览器可能需要显式调用）
          e.load();
          // 设置视频初始音量为 0.5
          e.volume = 0.5;
          // 停止观察该元素（避免重复触发）
          o.unobserve(e);
          // 绑定视频控制逻辑（自定义函数，比如播放/暂停按钮等）
          bindVideoControls();
        }
      });
    },
    {
      // 配置项：提前 200px 开始监听（元素距离视口 200px 时就触发加载）
      rootMargin: "200px",
      // 配置项：元素可见比例达到 0.08（8%）时判定为"进入视口"
      threshold: 0.08,
    }
  );
